...
def building_id_at(tile: Position, ct: Controller) -> int | None:
...
class Player:
    def __init__(self):
        ...
        self.chain_truncated: bool = False
    ...
    def _run_core(self, ct: Controller) -> None:
        ...
        economy_open = self.orders_issued < 5
        if 0 and ct.get_current_round() >= 0:
        ...
        if self.build_stage == 0:  # Has not yet built first conveyor
            if bot_position == self.go_to:  # Make any move:
                ...
                if ct.can_build_conveyor(self.go_to, self.conveyor_path[self.path_index]):
                    ...
                    blocked_by_id = building_id_at(bot_position.add(self.build_direction), ct)
    ...
    def _siege_going_nowhere(self, ct: Controller) -> bool:
        """True once the builder has spent a 20-round window inside two or three
        ...
        if len(self.siege_marks) > 20:
            self.siege_marks.pop(0)
        return len(self.siege_marks) >= 20 and len(set(self.siege_marks)) <= 3
