...
class Player:
    def __init__(self):
        ...
        self.orders_issued = 0  # Economy cap: limit total harvester orders to 5
    ...
    def _run_core(self, ct: Controller) -> None:
        ...
        attempted_ores = []
        economy_open = self.orders_issued < 5  # Cap at 5 harvester orders total
        for i in range(0,3-len(self.build_orders_made)):
            if not ENV_MAP.unplanned_ore: break
            if not economy_open: break  # Stop planning when economy cap reached
            ...
            if not possible:
                ...
                self.orders_issued += 1  # Increment counter
                economy_open = self.orders_issued < 5  # Check cap again
